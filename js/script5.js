...
burgerButton.addEventListener('click', function ()  {
    ...
    if(burgDecor.classList.length<2){
        burgDecor.classList.toggle('burger__dec__active');
        
    }
    else{
        burgDecor.classList.toggle('burger__dec__hide');
        burgDecor.classList.toggle('burger__dec__active');
...
validationButton[0].addEventListener('click', function ()  {
    ...
    for (let i = 0; i < formItems.length; i++) {
        ...
        if(formItems[i].value == ''){
            validated = false;
            formItems[i].classList.add('apply__not__validated');
        }
        else{
            formItems[i].classList.remove('apply__not__validated');
        }
    ...
    if (validated == true) {
        applyPopUp[0].classList.add('apply__pop__show');
        applyPopUp[0].classList.remove('apply__pop__hide');
        ...
        applyPopUp[1].classList.add('apply__pop__show');
        applyPopUp[1].classList.remove('apply__pop__hide');
    }
...
stateSuccesButton.addEventListener('click', function ()  {
    applyPopUp[0].classList.remove('apply__pop__show');
    applyPopUp[0].classList.add('apply__pop__hide');
    setTimeout(succAnim,200);
...
popWrongBtn.addEventListener('click', function ()  {
    applyPopUp[1].classList.remove('apply__pop__show');
    applyPopUp[1].classList.add('apply__pop__hide');
});
